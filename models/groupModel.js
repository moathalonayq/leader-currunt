...
/* -------- كل المجموعات مع إحصائياتها مرتبة حسب متوسط كيلوات الطالب --------
   "إجمالي المجموعة" هنا هو متوسط كيلوات أعضائها (إجمالي كيلوات المجموعة ÷ عدد
   طلابها) وليس مجموع الكيلوات الخام، حتى لا تتأثر المجموعات الأقل عدداً
...
async function getRankedGroups() {
  const [rows] = await pool.query(`
    SELECT
      ...
      COALESCE(SUM(s.knowledge_points + s.attendance_points), 0) AS raw_total_points
    FROM \`groups\` g
...
/* -------- أعضاء مجموعة معينة مرتبين حسب كيلواتهم --------
   ملاحظة: الترتيب (ORDER BY) يحتسب كيلوات المبادرة أيضاً كي يتطابق
   مع ترتيب الطالب الظاهر في ملفه الشخصي، لكن عمود "الإجمالي"
   المعروض هنا يبقى بدون المبادرة (الذاتي + حضور فقط) */
async function getGroupMembers(groupId) {
  const [rows] = await pool.query(`
    ...
    FROM students s
    WHERE s.group_id = ?
    ORDER BY (s.knowledge_points + s.attendance_points
