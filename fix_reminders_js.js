...
// I will replace the block of sendAllBtn with newJs.
code = code.replace(/const sendAllBtn = document\.getElementById\("sendAllBtn"\);\s*if \(sendAllBtn\) \{[\s\S]*?msg\.className = "form-msg success";/m, newJs + "\n          // ");

fs.writeFileSync("views/weekly-reminders.ejs", code);
console.log("Fixed weekly-reminders JS");
